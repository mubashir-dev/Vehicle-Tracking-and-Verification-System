		...
		<div class="card shadow mb-4">
						...
						<tbody>
							...
							if (!empty($rows)) { ?>
								...
								<?php foreach ($rows as $row) { ?>
									<?php $data['row'] = $row; ?>
									<?php $this->load->view('faculty/single_row', $data); ?>
								<?php }
							} else { ?>
			...
			<div class="modal fade" id="updateFacultyModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="exampleModalCenterTitle">Edit Faculty</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
			...
			<!--FacultyDetails Dialog-->

			<div class="modal" tabindex="-1" role="dialog" id="faculty_details">
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title">Faculty Details</h5>
							<button type="button" class="close" data-dismiss="#faculty_details" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
						<div class="modal-body" id="faculty_details_body">
			...
			<script>
				function showModal() {
					...
					$.ajax({
						url: '<?php echo base_url() . 'index.php/FacultyController/showCreateForm'; ?>',
						method: 'POST',
						data: {},
						dataType: 'json',
					...
					$("body").on("submit", "#CreateFacultyModel", function(e) {
						e.preventDefault();
						$.ajax({
							url: '<?php echo base_url() . 'index.php/FacultyController/saveFacultyModel'; ?>',
							method: 'POST',
							data: $("#CreateFacultyModel").serializeArray(),
							dataType: 'json',
							success: function(response) {
				...
				function showUpdateForm(id) {
					...
					$.ajax({
						url: '<?php echo base_url() . 'index.php/FacultyController/updateFacultyForm/'; ?>' + id,
						method: 'POST',
						data: {},
						dataType: 'json',
						success: function(response) {
				...
				function updateFaculty() {
					$.ajax({
						url: '<?php echo base_url() . 'index.php/FacultyController/updateFacultyModel'; ?>',
						method: 'POST',
						data: $("#UpdateFacultyModel").serializeArray(),
						dataType: 'json',
						success: function(response) {
							if (response['status'] == 0) {
								if (response['name'] != "") {
				...
				function deleteRecored(id) {
					...
					$("body").on('click', '#delete_button', function(e) {
						$.ajax({
							url: '<?php echo base_url() . 'index.php/FacultyController/deleteFacultyModel/'; ?>' + id,
							method: 'POST',
							data: {},
							dataType: 'json',
							success: function(response) {
