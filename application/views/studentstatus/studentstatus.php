<div class="container-fluid">
	<div class="row">
				...
				<div class="card-header card-header-primary bg-primary text-white">
					<div class="row">
						<div class="col-md-8">
							...
							<p class="card-category">Current Student Status who are using bus service</p>
						</div>
						<div class="col-md-4 text-right">
							<button class="btn btn-lg btn-success"  onclick="StudentStatusData()">Load Data</button>
						</div>
					</div>
				</div>
				<div class="card-body">
							...
							<tbody>
								...
								if (!empty($rows)) { ?>
									...
									<?php foreach ($rows as $row) { ?>
										<?php $data['row'] = $row; ?>
										<?php $this->load->view('studentstatus/single_row', $data); ?>
									<?php }
								} else { ?>
		...
		<script>
			function StudentStatusData() {
				$.ajax({
					url: '<?php echo base_url() . 'index.php/SStatusController/StudentStatusData'; ?>',
					method: 'POST',
					data: {},
					dataType: 'json',
			...
			function payFacultyFee(id) {
				// alert(id);
				$.ajax({
					url: '<?php echo base_url() . 'index.php/SStatusController/payFacultyFee'; ?>',
					method: 'POST',
					data: {
						id: id
					},
