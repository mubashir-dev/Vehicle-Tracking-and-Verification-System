...
class FFeeController extends CI_Controller
{
	...
	//Load Faculty Fee Data
	public function getFacultyFeeData()
	{

		$Faculty_data = $this->FFeeModel->getFacultyData();
		//	$data['student'] = $Student_data;

		if (!empty($Faculty_data)) {
			foreach ($Faculty_data as $row) {
				$route = $this->FFeeModel->getRouteCharges($row['route_id']);
				$data['fee_amount'] = 12000;
				$data['faculty_id'] = $row['faculty_id'];
				$data['fee_date'] = date('dd-mm-yyyy');
				$data['status'] = false;

				$this->FFeeModel->loadData($data);

				$faculty_data = array();
				$faculty_data['fee_data_link'] = 1;
				$this->FacultyModel->updateFeeLink($row['faculty_id'], $faculty_data);
			}
			$response['message'] = "<div class='alert alert-success'>Faculty Recored Added </div>";
		} else {
	...
	//Pay Faculty Fee
	function payFacultyFee()
	{
		$id = $this->input->post('id');
		$data['fee_date'] = date('Y-m-d');
		$data['status'] = true;
		$this->FFeeModel->feeFacultyPay($id, $data);
		$response['message'] = "<div class='alert alert-success'>Fee Has Been Successfully Payed</div>";
