<?php
class SStatusController extends CI_Controller
{
	...
	public function StudentStatusData()
	{
		...
		$Student_data = $this->StudentModel->getAllStudent();
		...
		if (!empty($Student_data)) {
			foreach ($Student_data as $row) {
				...
				$data['status']=true;
				$this->StudentModel->CreateStudentStatus($data);
				// $student_data=array();
				// $student_data['fee_data_link']=1;
				// // $this->StudentModel->updateFeeLink($row['student_id'],$student_data);

			}
			$response['message'] = "<div class='alert alert-success'>Student Status Recored Created </div>";
		} else {
