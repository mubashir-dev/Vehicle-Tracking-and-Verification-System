<?php
class FStatusController extends CI_Controller
{
	...
	//Load Faculty  Status  Form
	public function FacultyStatusData()
	{

		$Faculty_data = $this->FacultyModel->getAllFaculty();
		...
		if (!empty($Faculty_data)) {
			foreach ($Faculty_data as $row) {
				...
				$data['status']=true;
				$this->FacultyModel->CreateFacultyStatus($data);

			}
			$response['message'] = "<div class='alert alert-success'>Faculty Status Recored Created </div>";
		} else {
